...
public class ReferenceLoosePartFeeder extends ReferenceFeeder {
     ...
     */
    private List<RotatedRect> filterDimensionRotatedRect(List<RotatedRect> results, double ratio,
            double areaPixels) {
        ...
        filteredResults.removeIf((a) -> {
            ...
            if (pcentRatio > PERCENT_RATIO) {
                logger.debug("Ratio measured = " + ratioC + " waited=" + ratio
                        + "... footprint suppressed");
            }
    ...
    private double getBodyArea(Footprint footprint) {
        return footprint.getBodyWidth() * footprint.getBodyHeight();
    }

    private double getMinusRatio(Footprint footprint) {
        double ratio = footprint.getBodyHeight() / footprint.getBodyWidth();
        if (ratio < 1.0) {
            return ratio;
        }
        return 1 / ratio;
    ...
    private Location getPickLocation(Camera camera) throws Exception {
        ...
        // FCA @TODO if the part has a big value of height, the focal length is not ridiculous and
        // the size of
        // rectangle increase.
        // For the test purpose, a factor of correction is given with an absolute value, but later,
        // a method
        ...
        final double unitsPerSquarePixels = camera.getUnitsPerPixel().getY()
                * camera.getUnitsPerPixel().getX() * (correctionFocale * correctionFocale);
        final double areaPackagePixels = getBodyArea(pack.getFootprint()) / unitsPerSquarePixels;
        final double ratio = getMinusRatio(pack.getFootprint());
        ...
        List<RotatedRect> filteredResults =
                filterDimensionRotatedRect(results, ratio, areaPackagePixels);
        logger.debug("Number of rectangles filtered : " + results.size() + " ratio = " + ratio
                + " sizeArea=" + areaPackagePixels);
        ...
        if (filteredResults.isEmpty()) {
            double errorPcent = 0;
            if (!results.isEmpty()) {
                errorPcent = Math.round(Math
                        .abs((results.get(0).size.area() - areaPackagePixels) / areaPackagePixels));
            }
            throw new Exception(
                    "Feeder " + getName() + ": No parts found. near Area=" + errorPcent + " %");
        }
        ...
        for (RotatedRect rect : filteredResults) {
            double l2 = VisionUtils.getPixelLocation(camera, rect.center.x, rect.center.y)
                    .getLinearDistanceTo(camera.getLocation());
            // FCA @TODO Remove this absolute value. The goal is to keep only the nearest point
            // between camera and rectangle found.
            // Else, the camera can move to an other rectangle with a best area and ration each time
            // of iteration
        ...
        boolean isSquare = Math.abs(ratio - 1.0) < 0.1; // Si le ratio est <10%, ne plus considerer
                                                        // le rectangle
        if (isSquare) {
            ...
            if ((result.size.width < result.size.height)) // FCA l'angle d'un rectangle 0° = cote
                                                          // long
            {
                angle -= 90;
